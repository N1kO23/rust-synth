...
pub fn sawtooth_wave(
  ...
  for i in 0..(sample_rate as u32) {
    ...
    samples.push(amplitude * ((start_phase + (pos / size)) % 1.0 - 0.5));
  }
  return samples;
}
...
pub fn sine_wave(amplitude: f32, frequency: f32, start_phase: f32, sample_rate: f32) -> Vec<f32> {
  ...
  for i in 0..(sample_rate as u32) {
    let size = sample_rate / frequency;
    let pos = i as f32 % size;
    samples.push(amplitude * ((start_phase + pos / size) * (2.0 * PI) - 0.5).sin());
  }
  return samples;
}
...
pub fn triangle_wave(
  ...
  for i in 0..(sample_rate as u32) {
    let size = sample_rate / frequency;
    let pos = i as f32 % size;
    samples.push(amplitude * (((start_phase + (pos / size)) % 1.0 * 2.0 - 1.0).abs() - 0.5));
  }
  return samples;
}
